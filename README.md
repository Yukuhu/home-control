...
The `Build and test` job runs the Gradle build and scans with SonarCloud. On pull requests,
it waits for the quality gate, so a failed scan, failed gate, or missing `SONAR_TOKEN`
makes the required check red. Main-branch scans update the analysis baseline without
waiting for the gate. Keep `SONAR_TOKEN` in both the GitHub Actions and Dependabot
secret stores. GitHub does not provide that secret to fork pull requests, so their build
stays red until a separate scan path is configured.
